...
from factors import app

def update_destdb(destsession, table_name, sets, is_trade = True):
...
def yearly_quintile(destsession, session, factor_name, task_id, df):
    ...
    update_destdb(destsession, 'yearly_quantile',new_df, is_trade=False)
...
def industry_ir(destsession, session, factor_name, task_id, total_data):
    ...
    update_destdb(destsession, 'industry_ir',industry_ir_df, is_trade=False)
...
def ic_decay(destsession, session, factor_name, task_id, total_data):
    ...
    update_destdb(destsession, 'ic_decay',tvalues, is_trade=False)
...
def basic_info(destsession, session, factor_name, task_id, fac_rets_series,
    ...
    basic_info = {'fac_rets':fac_rets,'t_rets':t_rets,'ic_mean':ic_mean,
             ...
             'update_time':datetime.datetime.now().strftime('%Y-%m-%d %H:%S:%M'),'params':str(json.dumps(kwargs)),
             'remark':'中证500股票池'}
...
def factor_analysis(**kwargs):
    ...
    total_data['trade_date'] = total_data['trade_date'].apply(lambda x:datetime.datetime.fromtimestamp(x/1000).date())
    ##写入数据库
